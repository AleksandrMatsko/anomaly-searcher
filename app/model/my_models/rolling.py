...
class RollingModel(anomaly.base.AnomalyDetector):
    def __init__(self, 
                 model_create_func : Callable[[], anomaly.base.AnomalyDetector],
                 num_models : int,
                 model_ttl : int,
                 time_shift : int,
        ...
        self.__models = [self.model_create_func() for _ in range(num_models)]
    ...
    def learn_one(self, *args, **learn_kwargs) -> None:
        for idx, model in enumerate(self.__models):
            ...
            if self.__ttls[idx] == self.model_ttl:
                self.__models[idx] = self.model_create_func()
                self.__ttls[idx] = 0
